...
class PredictPipeline:
    def __init__(self, model_dir="../models/model_12_bulan_v5"):
        """
        Loads the pre-calculated predictions into server memory on startup.
        Takes 0 CPU power and handles thousands of requests instantly.
        """
        json_path = os.path.join(model_dir, "precalculated_forecasts_2027.json")
        print(f"Loading static predictions from {json_path}...")
        ...
        print(f"✅ Backend API Ready! Loaded data for {len(self.predictions)} rooms.")

    def predict(self, room_id, property_id=None):
        """Simulates an API endpoint: GET /api/v1/forecast/2027?roomId=X"""

        # JSON keys are always strings, so we ensure the room_id is a string
        clean_room_id = str(room_id)

        # Instant O(1) Dictionary Lookup
        forecast = self.predictions.get(clean_room_id)

        if not forecast:
            return {
                "error": f"Room ID {room_id} not found in pre-calculated 2027 inventory."
            }
